...
DynologTensorBoardLogger::DynologTensorBoardLogger(const std::string& metric_log_dir) : logPath_(metric_log_dir)
{
...
void DynologTensorBoardLogger::finalize()
{
    ...
    std::string real_tag = kind == "Marker" ? kind + "/" + dynamic_metrics_["domain"] : kind;
...
bool DynologTensorBoardLogger::validateLogDir(const std::string& path)
{
    ...
    for (auto& item : INVALID_CHAR)
    {
        ...
        {
            LOG(WARNING) << "The path contains invalid character: " << item.second;
        }
    }
    ...
    if (!std::filesystem::exists(log_path))
    {
        ...
        if (ec)
        {
            LOG(ERROR) << "Failed to create log dir: " << log_path << ", error: " << ec.message();
            return false;
        }
        LOG(INFO) << "Create log dir: " << log_path;
    }
    ...
    {
        LOG(WARNING) << "Path is a symbolic link: " << path;
    }
...
{
    static std::shared_ptr<TensorBoardLoggerManager> manager_ = std::make_shared<TensorBoardLoggerManager>();
    return manager_;
}
...
void TensorBoardLoggerManager::log(const MsptiMetricDesc& desc)
{
    ...
    if (it == device_loggers_.end())
    {
        ...
        logger = std::make_shared<TensorBoardLoggerImpl>(device_log_path, "");
        device_loggers_.insert({device, logger});
    }
    else
    {
        logger = it->second;
...
void TensorBoardLoggerImpl::log(const std::string& key, double val, uint64_t step)
{
    if (!std::filesystem::exists(log_path_))
    {
        ...
        if (ec)
        {
            LOG(ERROR) << "Failed to create log dir: " << log_path_ << ", error: " << ec.message();
            return;
        }
    }
