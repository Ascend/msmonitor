...
class EndPoint final
{
   public:
    ...
    explicit EndPoint(std::string address)
    {
        ...
        if (addr.sun_path[0] != '\0')
        {
            ...
            if (PathUtils::Exist(addr.sun_path))
            {
                ...
                {
                    throw std::runtime_error(std::string("Permission denied to delete existing socket file: ") +
                                             addr.sun_path);
                }
                if (unlink(addr.sun_path) != 0)
                {
                    throw std::runtime_error(std::string("Failed to delete existing socket file: ") +
                                             std::strerror(errno));
                }
            }
        }
        ...
        if (addr.sun_path[0] != '\0')
        {
            ...
            {
                throw std::runtime_error(std::string("Failed to set socket path permission: ") + std::strerror(errno));
            }
        }
    }
    ...
    [[nodiscard]] bool trySendMsg(EndPointCtxt const &ctxt, bool retryOnConnRefused = true)
    {
        ...
        // Enforce correct number of bytes sent.
        if (ret > 0)
        {
            return true;
        }
    ...
    [[nodiscard]] auto buildRcvCtxt(const std::vector<Payload> &payload) { return buildCtxt_(payload); }
    ...
    [[nodiscard]] bool tryRcvMsg(EndPointCtxt &ctxt) noexcept
    {
        ...
        // Enforce correct number of bytes received.
        if (ret > 0)
        {
            return true;
        }
    ...
    const char *getName(EndPointCtxt const &ctxt, bool is_abstract) const
    {
        if (is_abstract)
        {
            ...
            {
                throw std::runtime_error("Expected abstract socket, got " + std::string(ctxt.msg_name.sun_path));
            }
        ...
        else
        {
            ...
            if (!home_path.empty())
            {
                ...
                {
                    throw std::runtime_error("Unexpected socket name: " + std::string(ctxt.msg_name.sun_path) +
   ...
   protected:
    ...
    size_t setAddress_(const std::string &src, struct sockaddr_un &dest, bool is_abstract)
    {
        ...
        if (is_abstract)
        {
            ...
            {
                throw std::invalid_argument("Abstract UNIX Socket path cannot be larger than kMaxNameLen");
            }
            dest.sun_path[0] = '\0';
            if (src.empty())
            {
    ...
    bool checkAndSetSocketPath_(const std::string &address)
    {
        ...
        auto socket_path =
            hostUid.empty() ? home_path + "/" + address + ".sock" : home_path + "/" + address + "_" + hostUid + ".sock";
